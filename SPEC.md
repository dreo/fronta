...
- Backward compatibility with older Fronta releases or schemas is not supported. Prefer simple current contracts over compatibility paths; follow the release-specific [upgrade procedure](docs/reference.md#database-initialization-and-upgrades).
...
- `async with fronta.subscribe(name, states=("succeeded", "failed", "cancelled"), types=None, settings=None, batch_size=256, backfill=None) as feed` upserts filters, completes any pending backfill, then yields batches from a dedicated connection. A batch holds row locks and exposes `events: list[TaskEvent(seq, id, type, state, attempt)]` and `conn`. Batch size is 1–1,000.
...
- Delivery is at least once until ack or retention expiry. Filters apply when the transition statement reads the subscription; ordinary registration does not backfill earlier/in-flight statements. Filter updates affect future events, not existing backlog. Closing a consumer preserves registration; `unsubscribe(name)` deletes registration and backlog transactionally after waiting for matching publishers, deliveries and backfill chunks, using registration row locks. An iterator ends if it observes the registration missing.
- `backfill=True` projects every retained matching task when creating a registration; finite nonnegative seconds or `timedelta` restrict terminal rows by `finished_at`, while requested queued/running rows are included regardless of age. `None`/`False` disable creation-time backfill, but every call resumes a persisted pending backfill. Invalid arguments fail before connecting. Existing completed registrations and filter changes never start another backfill.
- Registration uses one upsert: only a new name gets a backfill marker; conflicts update filters and return existing progress. After commit, each creating/resuming consumer captures other backends' granted exclusive virtual-XID locks in this database and polls that fixed set in autocommit until it clears. Later transactions do not extend the wait. No exclusive table lock, retry or jitter is used. Virtual locks exist before snapshots/real XIDs and persist through idle transactions, so ordinary repeatable-read callers are covered. `pg_stat_activity` supplies public PID/database scoping and optional diagnostics only: tracking settings, timestamps and monitoring grants are not correctness requirements. Prepared transactions and imported snapshots are deliberately outside the guarantee.
- Chunks run by alphabetical state then ascending id, with `FOR NO KEY UPDATE` on the registration. Events and the JSONB marker (`generation` UUID, `since` absolute database timestamp, `pending` state-to-id cursors) commit together. Each chunk verifies the captured registration generation, preventing an old consumer from clearing a replacement registration's marker. Concurrent resumers serialize, publishers/pullers proceed, and timeouts roll back a whole chunk. Current type filters apply to each chunk. INFO start/end/removed logs and DEBUG chunk logs expose progress.
- The barrier can wait indefinitely for any older transaction in the same database. Finish caller transactions and this process's own batches before awaiting a backfilling subscription. Warnings every five seconds name virtual IDs, PIDs, users, application names and transaction ages when visible. Cancellation leaves progress resumable with a fresh capture. Caller-owned enqueue needs read committed for subsequent statements to observe filter changes and unsubscribe.
- With stable filters, backfill and live events cover matching states after registration commits, except tasks purged before being scanned. Backfill projects current rows, not complete task histories. Live transitions during registration/backfill can duplicate `(id, attempt, state)` with distinct `seq`; reactions must dedupe that tuple during backfill or be idempotent per task. Backfilled events start retention at insertion; live/backfill ordering is unspecified. Register at deploy time and keep subscriptions across restarts.
- Run `fronta db init` before using any 0.6.0 client. It adds nullable `subscriptions.backfill` without a schema-version bump or changes to claim functions. All subscriptions require the column and raise `ConfigurationError` naming the upgrade command when it is missing; stats reads it directly. Older-schema and mixed-version operation are unsupported.
- Database reactions and acknowledgements on `batch.conn` commit atomically. External effects need idempotency: `(subscription, seq)` identifies live deliveries; backfill duplicates need the handling above. `(id, attempt, state)` can repeat after manual requeue. `get_task(id, conn=None)` reads the latest durable row, not an event snapshot, and raises `TaskNotFound` after purge.
...
- `stats()` returns due/scheduled queued counts, running counts and oldest due age for published types, plus each subscription's backlog, oldest event age and boolean `backfill_pending`. These are active-row scans, not constant-time counters.
...
- `fronta db init`: atomically installs idempotent table DDL, additive columns, versioned `claim_vN` and `fronta.meta.schema_version`; obsolete functions can be removed with `--prune`. Workers and servers reject an older schema version with installed/required versions and the init command. `fronta db sql` prints the same SQL for an administrator. Init does not lower a newer installed version. Stop old clients, initialize the database, then start the new release; retained obsolete functions are not a compatibility guarantee. No general migration framework in V1.
...
- Feed backfill: all/windowed snapshots, active-state inclusion, opt-in validation, unchanged existing registrations, multi-chunk progress, disconnect/timeout recovery, concurrent creators/resumers, type updates, old-snapshot/late-XID and idle-transaction races under read committed/repeatable read, cross-role and disabled tracking without monitoring grants, independent resume barriers, fixed wait sets, replacement generation checks, blocker warnings and worker progress during open reaction batches, unsubscribe coordination, fresh event retention, required CLI upgrade and retained history after restart. Randomized six-second continuous-load runs reconcile every terminal id (10 seeds on macOS, 50 on Linux); late-start and mid-backfill-restart fan-in workflows finalize exactly once. The opt-in million-row stress run records latency, gaps, virtual-lock poll cost, completions throughout a 30-second blocker and post-consumption vacuum/storage measurements.
...
- Feed backfill: off by default; 5,000 rows per state/chunk; virtual-lock polling every 250 ms, blocker warnings every 5 s; no overall wait deadline. Module constants, no new settings.
